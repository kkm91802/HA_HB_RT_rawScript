...
// // Convert signed binary (up to 32 bits)
// int signed_bin_to_dec(uint32_t value, int bit_len) {
...
//     if (!infile) {
//         printf("⚠️  Skipping missing file: %s\n", input_path);
//         return 0;
//     }
//     if (!outfile) {
...
//         if (strcmp(can_id, "0x2DA") == 0) { // Accelerometer
//             int start = 0;
//             for (int i = 0; i < 3; i++) {
//                 char buf[20];
...
//             strncpy(latest_time, timestamp, sizeof(latest_time) - 1);
//             latest_time[sizeof(latest_time) - 1] = '\0';
//             accel_ready = 1;
//         }
//         else if (strcmp(can_id, "0x2DB") == 0) { // Gyroscope
//             int start = 0;
//             for (int i = 0; i < 3; i++) {
//                 char buf[25];
